 ...
 * @since 2018-12-12
 */
class ChinaAreaUtilsTest {

	private static final Logger log = LoggerFactory.getLogger(ChinaAreaUtilsTest.class);
	...
	void getAllProvinces() {
		Set<Province> provinceSet = ChinaAreaUtils.getAllProvinces();
	...
	void getCityByName() {
		...
		Arrays.stream(cityNames).forEach(item -> {
			City city = ChinaAreaUtils.getCityByName(item);
	...
	void getCountyByName() {
		...
		Arrays.stream(countyNames).forEach(item -> {
			Set<County> counties = ChinaAreaUtils.getCountyByName(item);
	...
	void getProvinceByName() {
		...
		Arrays.stream(provinceNames).forEach(item -> {
			Province province = ChinaAreaUtils.getProvinceByName(item);
