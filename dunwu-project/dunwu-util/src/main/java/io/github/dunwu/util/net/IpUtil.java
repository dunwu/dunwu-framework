...
public class IpUtil {
    ...
    /**
     * 从String转换为InetAddress. IpString可以是ipv4 或 ipv6 string, 但不可以是域名. 先字符串传换为byte[]再调getByAddress(byte[])
     * ，避免了调用getByName(ip)可能引起的DNS访问.
     */
    ...
    /**
     * 从IPv4String转换为InetAddress. IpString如果确定ipv4, 使用本方法减少字符分析消耗 . 先字符串传换为byte[]再调getByAddress(byte[])
     * ，避免了调用getByName(ip)可能引起的DNS访问.
     */
    ...
    public static String intToIpv4String(int i) {
        return new StringBuilder(15).append((i >> 24) & 0xff)
                                    .append('.')
                                    .append(i >> 16 & 0xff)
                                    .append('.')
                                    .append((i >> 8) & 0xff)
