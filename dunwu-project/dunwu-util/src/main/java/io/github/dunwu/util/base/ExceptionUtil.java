 ...
 * (其中一些来自Common Lang ExceptionUtils) 3. 查找Cause(其中一些来自Guava Throwables) 4.
...
public class ExceptionUtil {
	 ...
	 * CheckedException会用UndeclaredThrowableException包裹，RunTimeException和Error则不会被转变. copy
	...
	/**
	 * 如果是著名的包裹类，从cause中获得真正异常. 其他异常则不变. Future中使用的ExecutionException 与
	 * 反射时定义的InvocationTargetException， 真正的异常都封装在Cause中 前面 unchecked()
	...
	/**
	 * copy from Netty, 为静态异常设置StackTrace. 对某些已知且经常抛出的异常,
	 * 不需要每次创建异常类并很消耗性能的并生成完整的StackTrace. 此时可使用静态声明的异常. 如果异常可能在多个地方抛出，使用本函数设置抛出的类名和方法名.
	 * <pre>
	 ...
	 * 如果不能控制StackTrace的生成，也不能控制它的打印端(如logger)，可用此方法暴力清除Trace. 但Cause链依然不能清除,
	 * 只能清除每一个Cause的StackTrace.
	 */
