 ...
 * 不过仅在String对象较大时才有明显效果，否则抵不上访问ThreadLocal的消耗. 当StringBuilder在使用过程中，会调用其他可能也使用StringBuilderHolder的子函数时，需要创建独立的Holder,
 * 否则会共同使用公共的Holder 注意：在Netty环境中，使用Netty提供的基于FastThreadLocal的版本
 */
public class StringBuilderHolder {
	...
	/**
	 * 获取公共Holder的StringBuilder. 当StringBuilder会被连续使用，期间不会调用其他可能也使用StringBuilderHolder的子函数时使用.
	...
	/**
	 * 获取独立Holder的StringBuilder. 重置StringBuilder内部的writerIndex, 而char[]保留不动.
	 */
