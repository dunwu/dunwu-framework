...
public class MockUtil {
	...
	public static String anyBoyFirstName() {
		String dictionary = "Dylan,Jacob,Ethan,Gabriel,Nicholas,Jack,Joshua,Caleb,Ryan,Andrew,Caden,Tyler,Michael,Jaden,Zachary,"
	...
	public static String anyGirlFirstName() {
		String dictionary = "Lily,Emily,Sophia,Isabella,Ava,Emma,Kaitlyn,Hannah,Hailey,Olivia,Sarah,Abigail,Madeline,Madison,Kaylee,"
	...
	public static String anyLastName() {
		String dictionary = "Smith,Jones,Williams,Taylor,Brown,Davies,Evans,Wilson,Thomas,Johnson,Roberts,Robinson,Thompson,Wright,"
				+ "Walker,White,Edwards,Hughes,Green,Hall,Lewis,Harris,Clarke,Patel,Jackson";
		String[] names = StringUtils.splitByWholeSeparator(dictionary, ",");
		return mock(names);
	}

	public static String anyChineseLastName() {
		String dictionary = "李,王,张,刘,陈,杨,黄,赵,周,吴,徐,孙,朱,马,胡,郭,林,何,高,梁,郑,罗,宋,谢,唐,韩,曹,许,邓,萧,冯,曾,程,蔡,彭,潘,袁,于,董,余,苏,叶,吕,魏,蒋,田,杜,丁,沈,姜,范,江,"
	...
	public static String anyChineseBoyFirstName() {
	...
	public static String anyChineseGirlFirstName() {
		int count = RandomUtils.nextInt(2, 4);
	...
	public static String anyChineseBoyName() {
		return anyChineseLastName() + anyChineseBoyFirstName();
	}

	public static String anyChineseGirlName() {
		return anyChineseLastName() + anyChineseGirlFirstName();
	}

	public static String anyChineseName() {
		boolean isGirl = RandomUtils.nextBoolean();
		return anyChineseName(isGirl);
	}

	public static String anyChineseName(boolean isGirl) {
		if (isGirl) {
			return anyChineseGirlName();
		}
		else {
			return anyChineseBoyName();
		}
	}
	 ...
	 *
	 * @see <a href=
	 * "https://baike.baidu.com/item/信息交换用汉字编码字符集?fromtitle=GB2312&fromid=483170">百度词条-GB2312</a>
	 */
	public static String anyChineseSimpleLetter() {
	...
	public static String anyChineseSimpleLetterString(int minLen, int maxLen) {
		int count = RandomUtils.nextInt(minLen, maxLen);
		StringBuilder sb = new StringBuilder();
		for (char index = 0; index < count; index++) {
			sb.append(anyChineseSimpleLetter());
		}
		return sb.toString();
	}

	public static String anyChineseLetterString(int minLen, int maxLen) {
		char begin = '\u4E00';
		char end = '\u9FA5';
		StringBuilder sb = new StringBuilder();
