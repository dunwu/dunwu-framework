...
/**
 * 随机数工具集. 1. 获取无锁的ThreadLocalRandom, 性能较佳的SecureRandom 2. 保证没有负数陷阱，也能更精确设定范围的nextInt/nextLong/nextDouble (copy from
 * Common Lang RandomUtils，但默认使用性能较优的ThreadLocalRandom，并可配置其他的Random) 3. 随机字符串 (via Common Lang RandomStringUtils)
 *
...
public class RandomUtil {
	...
	/**
	 * 返回min-max间的随机Long,可传入SecureRandom或ThreadLocalRandom. min必须大于0. JDK本身不具有控制两端范围的nextLong，因此参考Commons Lang
	 * RandomUtils的实现, 不直接复用是因为要传入Random实例
	 *
	...
	/**
	 * 返回min－max间的随机Long, 使用ThreadLocalRandom. min必须大于0.
	 */
	...
	/**
	 * 返回min到max的随机Int, 使用ThreadLocalRandom. min必须大于0.
	 */
	...
	/**
	 * 返回min到max的随机Int,可传入SecureRandom或ThreadLocalRandom. min必须大于0. JDK本身不具有控制两端范围的nextInt，因此参考Commons Lang
	 * RandomUtils的实现, 不直接复用是因为要传入Random实例
	 *
