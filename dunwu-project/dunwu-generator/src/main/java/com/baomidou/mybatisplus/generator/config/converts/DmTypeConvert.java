...
public class DmTypeConvert implements ITypeConvert {
	...
	class BasicDataTypeHandler implements Handler {
		...
		public DbColumnType getDbColumnType(String fieldType) {
			...
			/**
			 * 数值数据类型: NUMERIC,DECIMAL,DEC,MONEY,BIT,BOOL,BOOLEAN,INTEGER,INT,BIGINT
			 * TINYINT,BYTE,SMALLINT,BINARY,VARBINARY
			 */
			else if (fieldType.contains("numeric") || fieldType.contains("decimal")
					|| fieldType.contains("dec") || fieldType.contains("money")) {
				return DbColumnType.BIG_DECIMAL;
			}
			else if (fieldType.contains("bit") || fieldType.contains("bool")
			...
			else if (fieldType.contains("tinyint") || fieldType.contains("byte")
					|| fieldType.contains("smallint")) {
				return DbColumnType.INTEGER;
			}
			else if (fieldType.contains("binary") || fieldType.contains("varbinary")) {
