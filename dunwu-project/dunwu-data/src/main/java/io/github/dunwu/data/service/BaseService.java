...
public abstract class BaseService<T extends BaseEntity> implements IService<T> {
	...
	public PageResult<T> page(Pagination<T> pagination, Wrapper<T> queryWrapper) {
		IPage<T> resultPage = dao.page(PageUtil.transToMybatisPlusPage(pagination),
				queryWrapper);
	...
	public PageResult<Map<String, Object>> pageMaps(Pagination<T> pagination,
		...
		IPage<Map<String, Object>> resultPage = dao
				.pageMaps(PageUtil.transToMybatisPlusPage(pagination), queryWrapper);
		return ResultUtil.successPageResult(PageUtil.transToPagination(resultPage));
	}
