...
/**
 * 十六进制（简写为hex或下标16）在数学中是一种逢16进1的进位制，一般用数字0到9和字母A到F表示（其中:A~F即10~15）。<br> 例如十进制数57，在二进制写作111001，在16进制写作39。<br>
...
public class HexUtil {
    ...
    public static char[] encodeHex(String str, Charset charset) {
        return encodeHex(StringUtil.toBytes(str, charset), true);
    }
    ...
    public static String encodeHexStr(String data, Charset charset) {
        return encodeHexStr(StringUtil.toBytes(data, charset), true);
    }

    /**
     * 判断给定字符串是否为16进制数<br> 如果是，需要使用对应数字类型对象的<code>decode</code>方法解码<br> 例如：{@code Integer.decode}方法解码int类型的16进制数字
     *
     * @param value 值
     * @return 是否为16进制
     */
    ...
    /**
     * 将指定char值转换为Unicode字符串形式，常用于特殊字符（例如汉字）转Unicode形式<br> 转换的字符串如果u后不足4位，则前面用0填充，例如：
     *
     * <pre>
     * '我' =》\u4f60
     * </pre>
