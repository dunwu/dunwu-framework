...
/**
 * 数字工具类<br> 对于精确值计算应该使用 {@link BigDecimal}<br> JDK7中<strong>BigDecimal(double val)</strong>构造方法的结果有一定的不可预知性，例如：
 *
 * <pre>
 * new BigDecimal(0.1)
 * </pre>
...
public class NumberUtil {
    ...
    /**
     * 判断两个数字是否相邻，例如1和2相邻，1和3不相邻<br> 判断方法为做差取绝对值判断是否为1
     *
     * @param number1 数字1
     * @param number2 数字2
     * @return 是否相邻
    ...
    /**
     * 是否是质数（素数）<br> 质数表的质数又称素数。指整数在一个大于1的自然数中,除了1和此整数自身外,没法被其他自然数整除的数。
     *
     * @param n 数字
     * @return 是否是质数
     */
    ...
    /**
     * 创建{@link BigInteger}，支持16进制、10进制和8进制，如果传入空白串返回null<br> from Apache Common Lang
     *
     * @param str 数字字符串
     * @return {@link BigInteger}
     * @since 3.2.1
    ...
    /**
     * 平方根算法<br> 推荐使用 {@link Math#sqrt(double)}
     *
     * @param x 值
     * @return 平方根
     */
    ...
    /**
     * byte数组转int，使用大端字节序（高位字节在前，低位字节在后）<br> 见：http://www.ruanyifeng.com/blog/2016/11/byte-order.html
     *
     * @param bytes byte数组
     * @return int
     * @since 4.4.5
