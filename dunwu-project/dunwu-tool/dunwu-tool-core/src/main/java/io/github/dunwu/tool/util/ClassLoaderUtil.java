...
import io.github.dunwu.tool.lang.JarClassLoader;
import io.github.dunwu.tool.lang.SimpleCache;
...
public class ClassLoaderUtil {
    ...
    /**
     * 指定类是否被提供，使用默认ClassLoader<br> 通过调用{@link #loadClass(String, ClassLoader, boolean)}方法尝试加载指定类名的类，如果加载失败返回false<br>
    ...
    /**
     * 指定类是否被提供<br> 通过调用{@link #loadClass(String, ClassLoader, boolean)}方法尝试加载指定类名的类，如果加载失败返回false<br>
     * 加载失败的原因可能是此类不存在或其关联引用类不存在
     *
     * @param className   类名
    ...
    /**
     * 加载类，通过传入类的字符串，返回其对应的类名<br> 此方法支持缓存，第一次被加载的类之后会读取缓存中的类<br> 加载失败的原因可能是此类不存在或其关联引用类不存在<br> 扩展{@link
    ...
    /**
     * 加载类，通过传入类的字符串，返回其对应的类名，使用默认ClassLoader并初始化类（调用static模块内容和初始化static属性）<br> 扩展{@link Class#forName(String, boolean,
     * ClassLoader)}方法，支持以下几类类名的加载：
     *
     * <pre>
     * 1、原始类型，例如：int
     * 2、数组类型，例如：int[]、Long[]、String[]
    ...
    /**
     * 加载类，通过传入类的字符串，返回其对应的类名，使用默认ClassLoader<br> 扩展{@link Class#forName(String, boolean, ClassLoader)}方法，支持以下几类类名的加载：
     *
     * <pre>
     * 1、原始类型，例如：int
     * 2、数组类型，例如：int[]、Long[]、String[]
