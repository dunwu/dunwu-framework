...
import io.github.dunwu.tool.convert.BasicType;
...
import io.github.dunwu.tool.lang.ClassScanner;
import io.github.dunwu.tool.lang.Filter;
import io.github.dunwu.tool.lang.Singleton;
...
public class ClassUtil {
    ...
    /**
     * 获得ClassPath，将编码后的中文路径解码为原字符<br> 这个ClassPath路径会文件路径被标准化处理
     *
     * @return ClassPath
     */
    ...
    /**
     * 获得给定类所在包的名称<br> 例如：<br> com.xiaoleilu.hutool.util.ClassUtil =》 com.xiaoleilu.hutool.util
     *
     * @param clazz 类
     * @return 包名
     */
    ...
    /**
     * 获取完整类名的短格式如：<br> cn.hutool.core.util.StrUtil -》c.h.c.u.StrUtil
     *
     * @param className 类名
     * @return 短格式类名
     * @since 4.1.9
    ...
    public static String getShortClassName(String className) {
        ...
        final StringBuilder result = StringUtil.getStringBuilder();
     ...
     *
     * @param <T>                     对象类型
     * @param classNameWithMethodName 类名和方法名表达式，类名与方法名用<code>.</code>或<code>#</code>连接 例如：com.xiaoleilu.hutool.StrUtil.isEmpty
     *                                或 com.xiaoleilu.hutool.StrUtil#isEmpty
    ...
    /**
     * 检查目标类是否可以从原类转化<br> 转化包括：<br> 1、原类是对象，目标类型是原类型实现的接口<br> 2、目标类型是原类型的父类<br> 3、两者是原始类型或者包装类型（相互转换）
     *
    ...
    /**
     * 获得外围类<br> 返回定义此类或匿名类所在的类，如果类本身是在包中定义的，返回{@code null}
     *
     * @param clazz 类
     * @return 外围类
     * @since 4.5.7
    ...
    /**
     * 扫面包路径下满足class过滤器条件的所有class文件，<br> 如果包路径为 com.abs + A.class 但是输入 abs会产生classNotFoundException<br> 因为className 应该为
     * com.abs.A 现在却成为abs.A,此工具类对该异常进行忽略处理,有可能是一个不完善的地方，以后需要进行修改<br>
     *
     * @param packageName 包路径 com | com. | com.abs | com.abs.
