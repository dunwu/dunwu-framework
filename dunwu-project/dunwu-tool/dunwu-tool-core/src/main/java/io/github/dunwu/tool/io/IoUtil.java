...
/**
 * IO工具类<br> IO工具类只是辅助流的读写，并不负责关闭流。原因是流可能被多次读写，读写关闭后容易造成问题。
 *
 * @author xiaoleilu
 */
public class IoUtil {
    ...
    /**
     * 尝试关闭指定对象<br> 判断对象如果实现了{@link AutoCloseable}，则调用之
     *
     * @param obj 可关闭对象
     * @since 4.3.2
     */
    ...
    /**
     * 对比两个流内容是否相同<br> 内部会转换流为 {@link BufferedInputStream}
     *
     * @param input1 第一个流
     * @param input2 第二个流
    ...
    /**
     * 对比两个流内容是否相同，忽略EOL字符<br> 内部会转换流为 {@link BufferedInputStream}
     *
     * @param input1 第一个流
     * @param input2 第二个流
     * @return 两个流的内容一致返回true，否则false
    ...
    public static ByteArrayInputStream toStream(String content, Charset charset) {
        ...
        return toStream(StringUtil.toBytes(content, charset));
    }
