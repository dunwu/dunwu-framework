...
public class EnumUtil {
    ...
    /**
     * 字符串转枚举，调用{@link Enum#valueOf(Class, String)}<br> 如果无枚举值，返回默认值
     *
     * @param <E>          枚举类型泛型
     * @param enumClass    枚举类
     * @param value        值
    ...
    /**
     * 获得枚举类中所有的字段名<br> 除用户自定义的字段名，也包括“name”字段，例如：
     *
     * <pre>
    ...
    /**
     * 获得枚举名对应指定字段值的Map<br> 键为枚举名，值为字段值
     *
     * @param clazz     枚举类
     * @param fieldName 字段名，最终调用getXXX方法
    ...
    /**
     * 获取枚举字符串值和枚举对象的Map对应，使用LinkedHashMap保证有序<br> 结果中键为枚举名，值为枚举对象
     *
     * @param <E>       枚举类型
     * @param enumClass 枚举类
