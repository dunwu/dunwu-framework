...
/**
 * 范围生成器。根据给定的初始值、结束值和步进生成一个步进列表生成器<br> 由于用户自行实现{@link Steper}来定义步进，因此Range本身无法判定边界（是否达到end），需在step实现边界判定逻辑。
 *
 * <p>
...
public class Range<T> implements Iterable<T>, Iterator<T>, Serializable {
    ...
    /**
     * 步进接口，此接口用于实现如何对一个对象按照指定步进增加步进<br> 步进接口可以定义以下逻辑：
     *
     * <pre>
     * 1、步进规则，即对象如何做步进
    ...
    public interface Steper<T> {
        ...
        /**
         * 增加步进<br> 增加步进后的返回值如果为{@code null}则表示步进结束<br> 用户需根据end参数自行定义边界，当达到边界时返回null表示结束，否则Range中边界对象无效，会导致无限循环
         *
