...
public class ClassScanner implements Serializable {
    ...
    public ClassScanner(String packageName, Filter<Class<?>> classFilter, Charset charset) {
        ...
        this.packageNameWithDot = StringUtil.appendSuffixIfNot(packageName, StringUtil.DOT);
    ...
    /**
     * 扫描包路径下满足class过滤器条件的所有class文件，<br> 如果包路径为 com.abs + A.class 但是输入 abs会产生classNotFoundException<br> 因为className 应该为
     * com.abs.A 现在却成为abs.A,此工具类对该异常进行忽略处理<br>
     *
     * @param packageName 包路径 com | com. | com.abs | com.abs.
    ...
    private String subPathBeforePackage(File file) {
        ...
        return StringUtil.appendSuffixIfNot(filePath, File.separator);
    }
    // --------------------------------------------------------------------------------------------------- Private method end
}
