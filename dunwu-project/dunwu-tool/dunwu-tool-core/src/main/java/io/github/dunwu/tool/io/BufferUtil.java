...
/**
 * {@link ByteBuffer} 工具类<br> 此工具来自于 t-io 项目以及其它项目的相关部分收集<br> ByteBuffer的相关介绍见：https://www.cnblogs.com/ruber/p/6857159.html
 *
...
public class BufferUtil {
    ...
    public static ByteBuffer create(CharSequence data, Charset charset) {
        return create(StringUtil.toBytes(data, charset));
    }
    ...
    /**
     * 读取指定长度的bytes<br> 如果长度不足，则读取剩余部分，此时buffer必须为读模式
     *
    ...
    /**
     * 读取一行，如果buffer中最后一部分并非完整一行，则返回null<br> 支持的换行符如下：
     *
     * <pre>
     * 1. \r\n
     * 2. \n
    ...
    /**
     * 一行的末尾位置，查找位置时位移ByteBuffer到结束位置<br> 支持的换行符如下：
     *
     * <pre>
     * 1. \r\n
     * 2. \n
