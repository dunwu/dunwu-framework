...
public class IterUtil {
    ...
    public static <K, V> Map<K, V> fieldValueAsMap(Iterator<?> iter, String fieldNameForKey, String fieldNameForValue) {
        ...
        if (null != iter) {
            ...
            while (iter.hasNext()) {
                value = iter.next();
                result.put((K) ReflectUtil.getFieldValue(value, fieldNameForKey),
    ...
    /**
     * 字段值与列表值对应的Map，常用于元素对象中有唯一ID时需要按照这个ID查找对象的情况<br> 例如：车牌号 =》车
     *
     * @param <K>       字段名对应值得类型，不确定请使用Object
     * @param <V>       对象类型
     * @param iter      对象列表
    ...
    public static <T> String join(Iterator<T> iterator, CharSequence conjunction, String prefix, String suffix) {
        ...
        while (iterator.hasNext()) {
            ...
            if (ArrayUtil.isArray(item)) {
                sb.append(ArrayUtil.join(conjunction, prefix, suffix, ArrayUtil.boxing(item)));
