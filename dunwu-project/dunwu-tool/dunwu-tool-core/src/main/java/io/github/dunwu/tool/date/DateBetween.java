...
public class DateBetween implements Serializable {
    ...
    /**
     * 构造<br> 在前的日期做为起始时间，在后的做为结束时间，间隔只保留绝对值正数
     *
     * @param begin 起始时间
     * @param end   结束时间
     */
    ...
    public DateBetween(Date begin, Date end, boolean isAbs) {
        if (begin == null) { throw new IllegalArgumentException("Begin date is null!");}
        if (end == null) { throw new IllegalArgumentException("End date is null!");}
    ...
    public long betweenMonth(boolean isReset) {
        final Calendar beginCal = DateUtil.toCalendar(begin);
        final Calendar endCal = DateUtil.toCalendar(end);
