...
public class MapUtil {
    ...
    /**
     * 创建代理Map<br> {@link MapProxy}对Map做一次包装，提供各种getXXX方法
     *
     * @param map 被代理的Map
    ...
    /**
     * 过滤<br> 过滤过程通过传入的Editor实现来返回需要的元素内容，这个Filter实现可以实现以下功能：
     *
     * <pre>
    ...
    public static <K, V> String join(Map<K, V> map, String separator, String keyValueSeparator, boolean isIgnoreNull,
        String... otherParams) {
        final StringBuilder strBuilder = StringUtil.getStringBuilder();
    ...
    /**
     * 去除Map中值为{@code null}的键值对<br> 注意：此方法在传入的Map上直接修改。
     *
     * @param <K> key的类型
     * @param <V> value的类型
     * @param map Map
    ...
    /**
     * 重命名键<br> 实现方式为一处然后重新put，当旧的key不存在直接返回<br> 当新的key存在，抛出{@link IllegalArgumentException} 异常
     *
     * @param <K>    key的类型
     * @param <V>    value的类型
     * @param map    Map
    ...
    /**
     * 将已知Map转换为key为驼峰风格的Map<br> 如果KEY为非String类型，保留原值
     *
     * @param <K> key的类型
     * @param <V> value的类型
     * @param map 原Map
    ...
    /**
     * 创建Map包装类MapWrapper<br> {@link MapWrapper}对Map做一次包装
     *
     * @param <K> key的类型
     * @param <V> value的类型
     * @param map 被代理的Map
