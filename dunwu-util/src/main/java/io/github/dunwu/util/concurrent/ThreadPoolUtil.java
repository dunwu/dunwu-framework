...
public class ThreadPoolUtil {
     ...
     * 使用了Guava的工具类
     *
     * @see MoreExecutors#shutdownAndAwaitTermination(java.util.concurrent.ExecutorService, long,
     ...
     * maxSize以下的线程, 如果在keepAliveTime中都poll不到任务执行将会被结束掉, keeAliveTimeJDK默认为10秒.
     * JDK默认值60秒太高，如高达1000线程时，要低于16QPS时才会开始回收线程,
     * 因此改为默认10秒.
     */
     ...
     * TaskQueue中可能3次有锁的读取线程数量，改为只读取1次，这把锁也是这个实现里的唯一遗憾了。 https://github
    ...
    public static class QueuableCachedThreadPool extends java.util.concurrent.ThreadPoolExecutor {
        ...
        public void execute(Runnable command, long timeout, TimeUnit unit) {
            ...
            try {
                ...
                // not to re-throw this exception because this is only used to find out whether the pool is full, not
                // for a
                // exception purpose
                final QueuableCachedThreadPool.ControllableQueue queue =
                    (QueuableCachedThreadPool.ControllableQueue) super.getQueue();
                try {
