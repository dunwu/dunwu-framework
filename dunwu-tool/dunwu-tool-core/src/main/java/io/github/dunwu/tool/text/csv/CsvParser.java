            ...
            if (config.errorOnDifferentFieldCount) {
                if (firstLineFieldCount == -1) {
                    ...
                    throw new IORuntimeException(
                        String.format("Line %d has %d fields, but first line has %d fields", lineNo, fieldCount,
                            firstLineFieldCount));
                }
            }
            ...
            if (bufLen == localBufPos) {
                ...
                if (bufLen < 0) {
                    ...
                    if (localPreChar == config.fieldSeparator || localCurrentField.hasContent()) {
                        //剩余部分作为一个字段
                        currentFields.add(
                            StringUtil.unWrap(localCurrentField.toStringAndReset(), config.textDelimiter));
                    }
                    break;
                }
