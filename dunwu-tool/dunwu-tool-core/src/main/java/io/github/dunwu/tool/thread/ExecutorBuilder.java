...
import io.github.dunwu.tool.builder.Builder;
...
public class ExecutorBuilder implements Builder<ThreadPoolExecutor> {
    ...
    private static ThreadPoolExecutor build(ExecutorBuilder builder) {
        ...
        RejectedExecutionHandler handler =
            ObjectUtil.defaultIfNull(builder.handler, new ThreadPoolExecutor.AbortPolicy());
    ...
    /**
     * 使用{@link SynchronousQueue} 做为等待队列（非公平策略）<br> 它将任务直接提交给线程而不保持它们。当运行线程小于maxPoolSize时会创建新线程，否则触发异常策略
     *
     * @return this
     * @since 4.1.11
     */
    ...
    /**
     * 使用{@link SynchronousQueue} 做为等待队列<br> 它将任务直接提交给线程而不保持它们。当运行线程小于maxPoolSize时会创建新线程，否则触发异常策略
     *
