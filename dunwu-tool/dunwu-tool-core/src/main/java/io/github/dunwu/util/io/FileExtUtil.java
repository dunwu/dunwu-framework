...
import io.github.dunwu.util.number.UnitConvertUtil;
...
public class FileExtUtil {
     ...
     */
    private static final long FILE_COPY_BUFFER_SIZE = UnitConvertUtil.SIZE_MB * 30;
    ...
    public static void moveFile(final File srcFile, final File destFile) throws IOException {
        ...
        if (!rename) {
            ...
            if (!srcFile.delete()) {
                FileExtUtil.deleteQuietly(destFile);
    ...
    public static String normalizePath(String path) {
        if (File.separator.equals(SystemExtUtil.WINDOWS_FILE_PATH_SEPARATOR)
            && StringUtils.indexOf(path,
            SystemExtUtil.LINUX_FILE_PATH_SEPARATOR) != -1) {
            return StringUtils.replaceChars(path,
                SystemExtUtil.LINUX_FILE_PATH_SEPARATOR,
                SystemExtUtil.WINDOWS_FILE_PATH_SEPARATOR);
        }
        return path;
    }
    ...
    public static final String readString(final File file) throws IOException {
        ...
        return StringExtUtil.join(lines, "\n");
    }
    ...
    private static final class FileExtensionFilter implements Predicate<File> {
        ...
        @Override
        public boolean apply(File file) {
            return file.isFile() && extension.equals(FileExtUtil.getFileExtension(file));
        }
