...
/**
 * 抽象转换器，提供通用的转换逻辑，同时通过convertInternal实现对应类型的专属逻辑<br> 转换器不会抛出转换异常，转换失败时会返回{@code null}
 *
 * @author Looly
 */
public abstract class AbstractConverter<T> implements Converter<T>, Serializable {
    ...
    public T convert(Object value, T defaultValue) {
        ...
        if (null == defaultValue || targetType.isInstance(defaultValue)) {
            ...
            throw new IllegalArgumentException(
                StringUtil.format("Default value [{}] is not the instance of [{}]", defaultValue, targetType));
        }
    }

    /**
     * 内部转换器，被 {@link AbstractConverter#convert(Object, Object)} 调用，实现基本转换逻辑<br> 内部转换器转换后如果转换失败可以做如下操作，处理结果都为返回默认值：
     *
     * <pre>
     * 1、返回{@code null}
    ...
    /**
     * 值转为String，用于内部转换中需要使用String中转的情况<br> 转换规则为：
     *
     * <pre>
     * 1、字符串类型将被强转
     * 2、数组将被转换为逗号分隔的字符串
