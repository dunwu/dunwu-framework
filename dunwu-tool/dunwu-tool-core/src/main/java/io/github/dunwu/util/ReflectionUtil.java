...
import io.github.dunwu.util.base.ExceptionExtUtil;
...
public class ReflectionUtil {
    ...
    /**
     * 先尝试用Getter函数读取, 如果不存在则直接读取变量. 性能较差, 用于单次调用的场景
     */
    ...
    public static <T> T invokeMethod(final Object obj, Method method, Object... args) {
        try {
            ...
            throw ExceptionExtUtil.unwrapAndUnchecked(e);
        }
    }

    /**
     * 直接读取对象属性值, 无视private/protected修饰符, 不经过getter函数. 性能较差, 用于单次调用的场景
     */
    ...
    /**
     * 使用已获取的Field, 直接读取对象属性值, 不经过getter函数. 用于反复调用的场景.
     */
        ...
        if (method == null) {
            throw new IllegalArgumentException(
                "Could not find method [" + methodName + "] with parameter types:"
                    + ObjectExtUtil.toPrettyString(parameterTypes)
    ...
    /**
     * 调用Setter方法, 无视private/protected修饰符, 按传入value的类型匹配函数. 性能较差, 用于单次调用的场景
     */
    ...
    /**
     * 先尝试用Setter函数写入, 如果不存在则直接写入变量, 按传入value的类型匹配函数. 性能较差, 用于单次调用的场景
     */
    ...
    /**
     * 使用预先获取的Field, 直接读取对象属性值, 不经过setter函数. 用于反复调用的场景.
     */
