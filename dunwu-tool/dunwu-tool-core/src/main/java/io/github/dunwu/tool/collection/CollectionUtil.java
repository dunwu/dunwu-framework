...
import io.github.dunwu.tool.comparator.PinyinComparator;
...
public class CollectionUtil {
    ...
    /**
     * 将指定对象全部加入到集合中<br> 提供的对象如果为集合类型，会自动转换为目标元素类型<br>
     *
     * @param <T>        元素类型
     ...
     * 结果：[c]，此结果中只保留了一个<br> 任意一个集合为空，返回另一个集合<br> 两个集合无差集则返回空集合
     *
     * @param <T>   集合元素类型
     * @param coll1 集合1
     * @param coll2 集合2
    ...
    /**
     * 如果提供的集合为{@code null}，返回一个不可变的默认空集合，否则返回原集合<br> 空集合使用{@link Collections#emptySet()}
     *
     * @param <T> 集合元素类型
    ...
    /**
     * 如果提供的集合为{@code null}，返回一个不可变的默认空集合，否则返回原集合<br> 空集合使用{@link Collections#emptyList()}
     *
     * @param <T> 集合元素类型
     * @param set 提供的集合，可能为null
     * @return 原集合，若为null返回空集合
    ...
    /**
     * 通过Editor抽取集合元素中的某些值返回为新列表<br> 例如提供的是一个Bean列表，通过Editor接口实现获取某个字段值，返回这个字段值组成的新列表
     *
    ...
    /**
     * 过滤，此方法产生一个新集合<br> 过滤过程通过传入的Editor实现来返回需要的元素内容，这个Editor实现可以实现以下功能：
     *
     * <pre>
    ...
    /**
     * 将多个集合排序并显示不同的段落（分页）<br> 采用{@link BoundedPriorityQueue}实现分页取局部
     *
     * @param <T>        集合元素类型
    ...
    /**
     * {@link Iterable}转为{@link Collection}<br> 首先尝试强转，强转失败则构建一个新的{@link ArrayList}
     *
     * @param <E>      集合元素类型
     * @param iterable {@link Iterable}
    ...
    /**
     * 映射键值（参考Python的zip()函数），返回Map无序<br> 例如：<br> keys = a,b,c,d<br> values = 1,2,3,4<br> delimiter = , 则得到的Map是 {a=1,
     * b=2, c=3, d=4}<br> 如果两个数组长度不同，则只对应最短部分
     *
     * @param keys      键列表
     * @param values    值列表
     * @param delimiter 分隔符
