package io.github.dunwu.tool.getter;
...
/**
 * 基本类型的getter接口抽象实现<br> 提供一个统一的接口定义返回不同类型的值（基本类型）<br> 在不提供默认值的情况下， 如果值不存在或获取错误，返回null<br> 用户只需实现{@link
 * OptBasicTypeGetter}接口即可
 *
 * @author Looly
 */
public interface OptNullBasicTypeGetter<K> extends BasicTypeGetter<K>, OptBasicTypeGetter<K> {
    ...
    /**
     * 获取boolean型属性值<br> 无值或获取错误返回null
     *
     * @param key 属性名
     * @return 属性值
     */
    ...
    /**
     * 获取BigDecimal型属性值<br> 无值或获取错误返回null
     *
     * @param key 属性名
     * @return 属性值
     */
    ...
    /**
     * 获取BigInteger型属性值<br> 无值或获取错误返回null
     *
     * @param key 属性名
     * @return 属性值
     */
