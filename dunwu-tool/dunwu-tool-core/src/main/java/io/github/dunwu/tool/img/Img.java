...
public class Img implements Serializable {
    ...
    public Img cut(Rectangle rectangle) {
        ...
        final Image image =
            Toolkit.getDefaultToolkit().createImage(new FilteredImageSource(srcImage.getSource(), cropFilter));
    ...
    public Img pressText(String pressText, Color color, Font font, int x, int y, float alpha) {
        ...
        g.drawString(pressText, Math.abs(targetImage.getWidth() - textLength) / 2 + x,
            Math.abs(targetImage.getHeight() + textHeight) / 2 + y);
        g.dispose();
        this.targetImage = targetImage;
