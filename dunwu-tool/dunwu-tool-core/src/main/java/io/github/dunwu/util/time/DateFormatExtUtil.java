 ...
 */
public class DateFormatExtUtil extends DateFormatUtils {
    ...
    public static String formatFriendlyTimeSpanByNow(final long timeStampMillis) {
        ...
        if (span < DateExtUtil.MILLIS_PER_SECOND) {
            return "刚刚";
        } else if (span < DateExtUtil.MILLIS_PER_MINUTE) {
            return String.format("%d秒前", span / DateExtUtil.MILLIS_PER_SECOND);
        } else if (span < DateExtUtil.MILLIS_PER_HOUR) {
            return String.format("%d分钟前", span / DateExtUtil.MILLIS_PER_MINUTE);
        }
        // 获取当天00:00
        long wee = DateExtUtil.beginOfDate(new Date(now)).getTime();
        if (timeStampMillis >= wee) {
            // 'R' 24 小时制的时间，被格式化为 "%tH:%tM"
            return String.format("今天%tR", timeStampMillis);
        } else if (timeStampMillis >= wee - DateExtUtil.MILLIS_PER_DAY) {
