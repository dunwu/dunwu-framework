...
public class DateFormatUtil {
    ...
    // 以T分隔日期和时间，并带时区信息，符合ISO8601规范
    public static final FastDateFormat ISO_FORMAT = FastDateFormat.getInstance(DatePattern.PATTERN_ISO.pattern());
    public static final FastDateFormat ISO_ON_SECOND_FORMAT = FastDateFormat.getInstance(
        DatePattern.PATTERN_ISO_ON_SECOND.pattern());
    public static final FastDateFormat ISO_ON_DATE_FORMAT = FastDateFormat.getInstance(
        DatePattern.PATTERN_ISO_ON_DATE.pattern());

    // 以空格分隔日期和时间，不带时区信息
    public static final FastDateFormat DEFAULT_FORMAT = FastDateFormat.getInstance(
        DatePattern.PATTERN_DEFAULT.pattern());
    public static final FastDateFormat DEFAULT_ON_SECOND_FORMAT = FastDateFormat.getInstance(
        DatePattern.PATTERN_DEFAULT_ON_SECOND.pattern());

    public static Date parseDate(@NotNull DatePattern datePattern, @NotNull String dateString) throws ParseException {
    ...
    public static String formatDate(@NotNull DatePattern datePattern, @NotNull Date date) {
    ...
    public static String formatFriendlyTimeSpanByNow(long timeStampMillis) {
        ...
        // 获取当天00:00
        long wee = DateUtil.beginOfDate(new Date(now))
                           .getTime();
