...
public class DunwuSchedulerAutoConfiguration {
     ...
     */
    @Bean(name = "persistScheduler")
    public Scheduler persistScheduler(SchedulerFactoryBean schedulerFactoryBean) throws SchedulerException {
    ...
    /**
     * 非持久化调度器，处理不需要持久化存储的调度任务，如临时性的调度任务或手动触发一次的持久化调度任务
     *
     * @param schedulerFactoryBean 调度器工厂类
     * @return 调度器实例
     * @throws SchedulerException 调度器异常
     */
    @Bean(name = "nonPersistentScheduler")
    public Scheduler nonPersistentScheduler(SchedulerFactoryBean schedulerFactoryBean) throws SchedulerException {
        Scheduler scheduler = schedulerFactoryBean.getScheduler();
        scheduler.start();
        return scheduler;
    }
