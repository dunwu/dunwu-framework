...
public class UserServiceTest {
    ...
    public void saveBatch() {
        ...
        for (int i = 1; i <= 10; i++) {
            User user = new User();
            user.setName(MockUtil.anyCBoyName()).setAge(RandomUtil.nextInt(10, 61)).setGender(GenderEnum.MALE)
                .setSalary(RandomUtil.nextDouble(0.0, 10000.0)).setEmail(MockUtil.anyEmail())
                .setRoleId(RandomUtil.nextLong(1L, 4L)).setDeleted(DeletedEnum.UNDELETE).setId((long) i);
            users.add(user);
        }
    ...
    public void test() {
        ...
        user.setName(MockUtil.anyCBoyName()).setAge(RandomUtil.nextInt(10, 61)).setGender(GenderEnum.MALE)
            .setSalary(RandomUtil.nextDouble(0.0, 10000.0)).setEmail(MockUtil.anyEmail())
            .setRoleId(RandomUtil.nextLong(1L, 4L)).setDeleted(DeletedEnum.UNDELETE).setId((long) 10);
