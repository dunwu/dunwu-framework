...
public class MailServiceImpl implements MailService {
    ...
    public boolean send(MailDTO mailDTO) {
        Future<?> future;
        if (mailDTO.getHtml()) {
            future = mailExecutorService.submit(() -> sendMimeMessage(mailDTO));
        } else {
            future = mailExecutorService.submit(() -> sendSimpleMessage(mailDTO));
        }
    ...
    public void sendBatch(List<MailDTO> mailDTOS, boolean html) {
        if (html) {
            mailExecutorService.submit(() -> sendMimeMessage(mailDTOS));
        } else {
            mailExecutorService.submit(() -> sendSimpleMessage(mailDTOS));
        }
    }
